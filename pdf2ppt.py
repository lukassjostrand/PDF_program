...
def make_slide_4_verify(presentation, verify_file):
	...
	color1_10 = RGBColor(255, 126, 121)
	color10_30 = RGBColor(255, 213, 121)
	color30_69 = RGBColor(255, 253, 120)
	color70_90 = RGBColor(167, 252, 103)
	color90_100 = RGBColor(107, 225, 82)
						...
						for row_index, row_cells in enumerate(target_table_cells):
							...
							if cell.text_frame.paragraphs:
								...
								if source_cell.text_frame.paragraphs:
									...
									if 0 < int(new_texts[row_index]) <= 9:
										cell.fill.solid()
										cell.fill.fore_color.rgb = color1_10
									if 10 <= int(new_texts[row_index]) <= 30:
										cell.fill.solid()
										cell.fill.fore_color.rgb = color10_30
									if 31 <= int(new_texts[row_index]) <= 69:
										cell.fill.solid()
										cell.fill.fore_color.rgb = color30_69
									if 70 <= int(new_texts[row_index]) <= 90:
										cell.fill.solid()
										cell.fill.fore_color.rgb = color70_90
									if 91 <= int(new_texts[row_index]) <= 100:
										cell.fill.solid()
										cell.fill.fore_color.rgb = color90_100
...
def make_slide_16(presentation, verify_file):
	x, y, name = string_to_list_verify(verify_file)
	slide_index = 15
	percent_list, text_list, name = string_to_list_verify(verify_file)
	color1_10 = RGBColor(255, 126, 121)
	color10_30 = RGBColor(255, 213, 121)
				...
				if len(table.rows) >= len(target_table_cells) and len(table.columns) >= 1:
					...
					if found_table:
						...
						if text_frame.paragraphs:
							...
							if 0 < int(percent_list[i+1]) <= 9:
								cell.fill.solid()
								cell.fill.fore_color.rgb = color1_10

							if 10 <= int(percent_list[i+1]) <= 30:
								cell.fill.solid()
								cell.fill.fore_color.rgb = color10_30

							if 31 <= int(percent_list[i+1]) <= 69:
								cell.fill.solid()
								cell.fill.fore_color.rgb = color30_69

							if 70 <= int(percent_list[i+1]) <= 90:
								cell.fill.solid()
								cell.fill.fore_color.rgb = color70_90

							if 91 <= int(percent_list[i+1]) <= 100:
								cell.fill.solid()
								cell.fill.fore_color.rgb = color90_100
...
def run(opq_file, verify_file, checkbox_list):
    ...
    # Save the modified presentation
    x, y,name = string_to_list_verify(verify_file)
    new_presentation_name = f"Fördjupad bedömning - {name}.pptx"
    modified_presentation_path = new_presentation_name
    presentation.save(modified_presentation_path)
